...
class CausalGaussianizationLayer(nn.Module):
    def __init__(self, nodes, datapoint_num = 10):
        super().__init__()
        self.nodes = nodes
        self.dim = len(nodes)
        self.datapoint_num = datapoint_num
        self.normal = dists.Normal(0, 1)
        self.datapoints = nn.Parameter(torch.randn(self.datapoint_num, self.dim))
    ...
    def normal_cdf(self, x, it = 1000):
        ...
        for i in range(it):
            mid = (low + high) / 2
            val = self.inverse_normal_cdf(mid, False)
    ...
    def inverse_normal_cdf(self, x, compute_logd = True):
        ...
        cdf_bad_right_log = log_ccdf * cdf_mask_right + -1. * (1 - cdf_mask_right)
        ...
        cdf_bad_left_log = log_cdf * cdf_mask_left + -1. * (1 - cdf_mask_left)
    ...
    def forward(self, u):
        x = u.clone()
        x[:, self.nodes], logd = self.normal_cdf(u[:, self.nodes])
        return x, logd

    def reward(self, x):
        u = x.clone()
        u[:, self.nodes], logd = self.inverse_normal_cdf(x[:, self.nodes])
        return u, logd
